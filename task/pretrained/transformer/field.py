...
import torch
from task.pretrained.transformer.vocab import TagVocab, BracketVocab
...
class BracketField(data.Field):
    def __init__(self, **kwargs):
        super(BracketField, self).__init__(**kwargs)
        self.vocab_cls = BracketVocab
    ...
    def build_vocab(self, *args, **kwargs):
        ...
        for data in sources:
            for x in data:
                x = [i for i in x if '*' not in i and i != 'O']
                counter.update(x)
        specials = list(OrderedDict.fromkeys(
            tok for tok in [self.unk_token, self.pad_token, self.init_token,
                            self.eos_token]
