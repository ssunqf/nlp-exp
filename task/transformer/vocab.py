...
class TagVocab(Vocab):
    def __init__(self, counter, **kwargs):
        ...
        self.begin_constraints = (self.begin_mask | self.single_mask)
        ...
        self.end_constraints = (self.end_mask | self.single_mask)
    ...
    def check_valid(self, mask):
        ...
        for i in range(seq_len):
            if i == 0:
                assert ((mask[i] == 1) | (self.begin_constraints == 1)).sum() > 0
            elif i == seq_len - 1:
                assert ((mask[i] == 1) | (self.end_constraints == 1)).sum() > 0

    def dense_mask(self, seqs: List[List[str]], batch_first: bool):
        batch_size = len(seqs)
        assert batch_size > 0
        seq_size = len(seqs[0])

        masks = torch.ones(batch_size, seq_size, len(self), dtype=torch.int8)
