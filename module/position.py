...
class TimeSignal(nn.Module):
    ...
    def __init__(self, dim, max_length=5000):
        ...
        self.embedding = torch.cat([times.sin(), times.cos()], - 1)

    def forward(self, input, batch_first=False):
        if batch_first:
            batch_first, max_len = input.size()
            return self.embedding[0:max_len].unsqueeze(0).expand(batch_first, -1, -1)
        else:
            max_len, batch_first = input.size()
            return self.embedding[0:max_len].unsqueeze(1).expand(-1, batch_first, -1)